...
def main():
    ...
    for i in range(TEST_EPISODE):
        task = tg.GestureTask(metatest_gesture_folders,CLASS_NUM,SAMPLE_NUM_PER_CLASS, SAMPLE_NUM_PER_CLASS, standardize=False, shuffle_all=True)
        sample_dataloader = tg.get_data_loader(task,num_per_class=SAMPLE_NUM_PER_CLASS,split="support",shuffle=False)
        test_dataloader = tg.get_data_loader(task,num_per_class=SAMPLE_NUM_PER_CLASS,split="query",shuffle=False)
        ...
        logits, fm = weighing_module(weighing_features)
        ...
        if SHOW_HEAT_MAP:
            ...
            plt.figure()
            ax = sb.heatmap(np.array(fm.data[:5].cpu()), linewidths=0.1, cmap='inferno') #inferno
            plt.axis('off')
            plt.show()
    ...
    # for j in range(16):
    #     for name, para in weighing_module.named_parameters():
    #         if name == 'fc1.0.weight':
    #             # para = para.view(-1, 64).contiguous().transpose(0,1)
    #             para = para[j].view(-1, 64).permute(1, 0)
    #             break
    #     plt.figure()
    #     ax = sb.heatmap(para.data.cpu(), linewidths=0.1, cmap='inferno')  # inferno
    #     plt.axis('off')
    #     plt.show()
