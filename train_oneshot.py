...
class WeighingNetwork(nn.Module):
    ...
    def forward(self,x):
        ...
        out = out.view(-1, CLASS_NUM*FEATURE_DIM)  #(?, 320)
        out = self.fc1(out) #(?, 10)
        out = self.fc2(out)  #(?, 5)
        return out
...
def main():
    ...
    if LOAD_EXISTED:
        if os.path.exists(str("./models/gesture_embedding_module_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")):
            embedding_module.load_state_dict(torch.load(str("./models/gesture_embedding_module_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")))
            print("load embedding network success")
        if os.path.exists(str("./models/gesture_weighing_module_"+ str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")):
            weighing_module.load_state_dict(torch.load(str("./models/gesture_weighing_module_"+ str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")))
    ...
    for episode in range(EPISODE):
        ...
        task = tg.GestureTask(metatrain_gesture_folders,CLASS_NUM,SAMPLE_NUM_PER_CLASS,BATCH_NUM_PER_CLASS, standardize=False)
        ...
        # calculate features
        sample_features = embedding_module(Variable(samples).cuda(GPU)) # 5x64x15x15
        batch_features = embedding_module(Variable(batches).cuda(GPU)) # 95x64x15x15

        # calculate probs
        sample_features_ext = sample_features.unsqueeze(0).repeat(BATCH_NUM_PER_CLASS*CLASS_NUM,1,1,1,1) # 95x5x64x15x15
        batch_features_ext = batch_features.unsqueeze(0).repeat(SAMPLE_NUM_PER_CLASS*CLASS_NUM,1,1,1,1)  # 5x95x64x15x15
        batch_features_ext = torch.transpose(batch_features_ext,0,1)

        weighing_features = torch.cat((sample_features_ext,batch_features_ext),2).view(-1,FEATURE_DIM*2,15,15) # 475x128x15x15
        # print(weighing_pairs.shape)
        ...
        if (episode+1)%1000 == 0:
            ...
            for i in range(TEST_EPISODE):
                task = tg.GestureTask(metaval_gesture_folders,CLASS_NUM,SAMPLE_NUM_PER_CLASS,SAMPLE_NUM_PER_CLASS, standardize=False)
                sample_dataloader = tg.get_data_loader(task,num_per_class=SAMPLE_NUM_PER_CLASS,split="support",shuffle=False)
                test_dataloader = tg.get_data_loader(task,num_per_class=SAMPLE_NUM_PER_CLASS,split="query",shuffle=True)
                ...
                # calculate features
                sample_features = embedding_module(Variable(sample_images).cuda(GPU)) # 5x64x15x15
                test_features = embedding_module(Variable(test_images).cuda(GPU)) # 5x64x15x15
                # print(1,sample_features.shape)
                # print(2,test_features.shape)
